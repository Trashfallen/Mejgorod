@echo off
rem Build Mejgorod.exe: icon and version info in resources, no console window.
rem Usage: build.cmd [output.exe]   (default dist\Mejgorod.exe)
setlocal
cd /d "%~dp0"
set "OUT=%~1"
...
go run github.com/tc-hib/go-winres@latest simply --icon winres/icon.png --manifest gui --arch amd64 ^
  ...
  --product-version %VERSION% --file-version %VERSION% --original-filename Mejgorod.exe --out rsrc || exit /b 1
